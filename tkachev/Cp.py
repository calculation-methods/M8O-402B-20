...
def quad_trapezoid_method(start, end, step_size, kernel, right_side):
    ...
    for i in range(1, num_intervals + 1):
        c_i = right_side(x_values[i]) + sum(
            step_size * kernel(x_values[i], x_values[j]) * y_values[j] * y_values[j] for j in range(0, i))
...
def newton_kantorovich(K, K_y, f, a, b, h, m):
    ...
    while k < m:
        for i in range(N + 1):
            F[i] = f(x[i]) + sum(
                h * (K(x[i], x[j], Y[j]) - K_y(x[i], x[j], Y[j]) * Y[j]) for j in range(1, N)) + h / 2 * (
                           K(x[i], x[0], Y[0]) + K(x[i], x[N], Y[N]) - K_y(x[i], x[0], Y[0]) * Y[0] - K_y(x[i], x[N],
                                                                                                          Y[N]) * Y[N])
        for i in range(N + 1):
